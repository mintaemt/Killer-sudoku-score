...
-- 2. 重新創建普通模式排行榜視圖（不使用 SECURITY DEFINER）
CREATE VIEW normal_leaderboard AS
SELECT 
  u.name,
  nr.difficulty,
...
-- 3. 重新創建多巴胺模式排行榜視圖（不使用 SECURITY DEFINER）
CREATE VIEW dopamine_leaderboard AS
SELECT 
  u.name,
  dr.difficulty,
